        ...
        <el-table :data="regionData" class="table" height="string" v-loading="listLoading">
          ...
          <el-table-column label="操作" width="300" align="center">
            <template slot-scope="scope">
              ...
              <el-button type="danger" size="small" @click="delectLRegion(scope.$index,scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
import { getRegionData,postRegionData,putRegionData,deleteRegionData } from "@/api/api";
    ...
    addSubmit() {
      ...
      delectLRegion(index, row) {
        this.$confirm('此操作将删除该户型选项', '提示', {
          ...
          let param = row.id
          this.listLoading = true
          deleteRegionData(param).then((res) => {
            // 公共提示方法
            common.statusinfo(this, res.data)
            this.getList()
          }).catch((err) => {
